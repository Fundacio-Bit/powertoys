...
@Table(name = "pwt_fitxer" , indexes = { 
        @Index(name="pwt_fitxer_pk_i", columnList = "fitxerid")})
@SequenceGenerator(name="FITXER_SEQ", sequenceName="pwt_fitxer_seq", allocationSize=1, initialValue=1000)
...
// EXP  Field:fitxerid | Table: pwt_ear | Type: 0  

    @OneToMany(fetch = FetchType.LAZY, mappedBy = "fitxerID")
    private Set<EarJPA> ears = new HashSet<EarJPA>(0);
    public  Set<EarJPA> getEars() {
    return this.ears;
  }

    public void setEars(Set<EarJPA> ears) {
      this.ears = ears;
    }


// EXP  Field:fitxerid | Table: pwt_earsimple | Type: 0  

    @OneToMany(fetch = FetchType.LAZY, mappedBy = "fitxerID")
    private Set<EarSimpleJPA> earSimples = new HashSet<EarSimpleJPA>(0);
    public  Set<EarSimpleJPA> getEarSimples() {
    return this.earSimples;
  }

    public void setEarSimples(Set<EarSimpleJPA> earSimples) {
      this.earSimples = earSimples;
    }
  ...
  void postLoad() {
    if (threadLocal.get() != null) {
      this.encryptedFileID = org.fundaciobit.powertoys.hibernate.HibernateFileUtil.encryptFileID(fitxerID);
    }
  }
 ...
 // ---------------  STATIC METHODS ------------------
  ...
  static FitxerJPA copyJPA(FitxerJPA __jpa,
    ...
    if(!"EarJPA".equals(origenJPA) 
       && ( !org.fundaciobit.genapp.common.utils.Utils.isEmpty(__jpa.ears) || org.hibernate.Hibernate.isInitialized(__jpa.getEars())) ) {
      __tmp.setEars(EarJPA.copyJPA(__jpa.getEars(), __alreadyCopied,"FitxerJPA"));
    }
    if(!"EarSimpleJPA".equals(origenJPA) 
       && ( !org.fundaciobit.genapp.common.utils.Utils.isEmpty(__jpa.earSimples) || org.hibernate.Hibernate.isInitialized(__jpa.getEarSimples())) ) {
      __tmp.setEarSimples(EarSimpleJPA.copyJPA(__jpa.getEarSimples(), __alreadyCopied,"FitxerJPA"));
    }
