...
@Tile(name="earSimpleFormWebDB", contentJsp="/WEB-INF/jsp/webdb/earSimpleForm.jsp", extendsTile=Tab.MENU_WEBDB,
      type=TileType.WEBDB_FORM , attributes={ @TileAttribute(name="titol", value="earSimple.earSimple")})
@Tile(name="earSimpleListWebDB", contentJsp="/WEB-INF/jsp/webdb/earSimpleList.jsp", extendsTile=Tab.MENU_WEBDB,
...
public class EarSimpleController
  ...
  public void fillReferencesForForm(EarSimpleForm earSimpleForm,
    ...
    if (earSimpleForm.getListOfValuesForJbossVersion() == null) {
      List<StringKeyValue> _listSKV = getReferenceListForJbossVersion(request, mav, earSimpleForm, null);

      if(_listSKV != null && !_listSKV.isEmpty()) { 
          java.util.Collections.sort(_listSKV, STRINGKEYVALUE_COMPARATOR);
      }
      earSimpleForm.setListOfValuesForJbossVersion(_listSKV);
    }
...
public java.lang.Long stringToPK(String value) {
 ...
 exceptionMsg };
  ...
  public List<StringKeyValue> getReferenceListForJbossVersion(HttpServletRequest request,
       ModelAndView mav, EarSimpleForm earSimpleForm, Where where)  throws I18NException {
    if (earSimpleForm.isHiddenField(JBOSSVERSION)) {
  ...
  public List<StringKeyValue> getReferenceListForJbossVersion(HttpServletRequest request,
       ModelAndView mav, EarSimpleFilterForm earSimpleFilterForm,
       List<EarSimple> list, Map<Field<?>, GroupByItem> _groupByItemsMap, Where where)  throws I18NException {
    if (earSimpleFilterForm.isHiddenField(JBOSSVERSION)
       && !earSimpleFilterForm.isGroupByField(JBOSSVERSION)
       && !earSimpleFilterForm.isFilterByField(JBOSSVERSION)) {
      return EMPTY_STRINGKEYVALUE_LIST;
    }
    Where _w = null;
