...
<un:useConstants var="EntornFields" className="org.fundaciobit.powertoys.model.fields.EntornFields"/>
  ...
  <div id="FilterDiv" class="wellgroupfilter formbox" style="${displayFilterDiv} margin-bottom:3px; margin-left: 1px; padding:3px;">
      ...
      <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,EntornFields.ENTORNID)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="entorn.entornID" />:</span>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,EntornFields.NOM)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="entorn.nom" var="nom" />
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,EntornFields.DOMINI)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="entorn.domini" var="domini" />
              <fmt:message key="genapp.form.searchby" var="cercaperdomini" >                
                 <fmt:param value="${domini}"/>
              </fmt:message>
              <span class="add-on"><c:out value="${domini}" />:</span>
              <form:input cssClass="search-query input-medium" placeholder="${cercaperdomini}" path="domini" />
            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,EntornFields.ORDRE)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="entorn.ordre" />:</span>
