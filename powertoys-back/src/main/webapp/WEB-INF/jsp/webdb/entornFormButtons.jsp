...
<%@ include file="/WEB-INF/jsp/moduls/includes.jsp"%>
  ...
  <div class="navbar-form" style="text-align:right">
    ...
    <input type="button" class="btn btn-secondary" onclick="goTo('<c:url value="${contexte}${__theForm.nou?'':'/'.concat(__theForm.entorn.entornID)}/cancel"/>')" value="<fmt:message key="genapp.cancel"/>">
    </c:if>
    <c:if test="${!__theForm.nou && __theForm.deleteButtonVisible}">
    <button type="button" class="btn btn-danger" onclick="openModal('<c:url value="${contexte}/${__theForm.entorn.entornID}/delete"/>','show');"><fmt:message key="genapp.delete"/></button>
    </c:if>
    ...
    <c:set var="pk" value="${__theForm.entorn.entornID}"/>
