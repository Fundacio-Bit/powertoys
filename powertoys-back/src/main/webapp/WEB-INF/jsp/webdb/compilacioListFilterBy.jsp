...
<un:useConstants var="CompilacioFields" className="org.fundaciobit.powertoys.model.fields.CompilacioFields"/>
  ...
  <div id="FilterDiv" class="wellgroupfilter formbox" style="${displayFilterDiv} margin-bottom:3px; margin-left: 1px; padding:3px;">
      ...
      <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,CompilacioFields.COMPILACIOID)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="compilacio.compilacioID" />:</span>

              <span class="add-on">&nbsp;<fmt:message key="genapp.from" /></span>
              
              <form:input cssClass="input-append input-small" path="compilacioIDDesde" />


              <span class="add-on">&nbsp;<fmt:message key="genapp.to" />&nbsp;</span>

              <form:input cssClass="input-append input-small search-query" path="compilacioIDFins" />

            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,CompilacioFields.REPOCOMPILACIOID)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="compilacio.repocompilacioID" />:</span>

              <span class="add-on">&nbsp;<fmt:message key="genapp.from" /></span>
              
              <form:input cssClass="input-append input-small" path="repocompilacioIDDesde" />


              <span class="add-on">&nbsp;<fmt:message key="genapp.to" />&nbsp;</span>

              <form:input cssClass="input-append input-small search-query" path="repocompilacioIDFins" />

            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,CompilacioFields.TAGURL)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="compilacio.tagUrl" var="tagUrl" />
              <fmt:message key="genapp.form.searchby" var="cercapertagUrl" >                
                 <fmt:param value="${tagUrl}"/>
              </fmt:message>
              <span class="add-on"><c:out value="${tagUrl}" />:</span>
              <form:input cssClass="search-query input-medium" placeholder="${cercapertagUrl}" path="tagUrl" />
            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,CompilacioFields.DATAINICI)}">
<%-- FILTRE DATE-TIME --%>
            <div class="input-group" style="padding-right:4px;padding-bottom:4px;align-items:center;">
              <span class="add-on"><fmt:message key="compilacio.dataInici" />:</span>
              <span class="add-on">&nbsp;<fmt:message key="genapp.from" /></span>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,CompilacioFields.DATAFI)}">
<%-- FILTRE DATE-TIME --%>
            <div class="input-group" style="padding-right:4px;padding-bottom:4px;align-items:center;">
              <span class="add-on"><fmt:message key="compilacio.dataFi" />:</span>
              <span class="add-on">&nbsp;<fmt:message key="genapp.from" /></span>
            <div class="form-group"  style="margin-bottom: 0px;" >
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,CompilacioFields.EXITCODE)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="compilacio.exitCode" />:</span>

              <span class="add-on">&nbsp;<fmt:message key="genapp.from" /></span>
              
              <form:input cssClass="input-append input-small" path="exitCodeDesde" />


              <span class="add-on">&nbsp;<fmt:message key="genapp.to" />&nbsp;</span>

              <form:input cssClass="input-append input-small search-query" path="exitCodeFins" />

            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,CompilacioFields.OUTPUT)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="compilacio.output" var="output" />
              <fmt:message key="genapp.form.searchby" var="cercaperoutput" >                
                 <fmt:param value="${output}"/>
              </fmt:message>
              <span class="add-on"><c:out value="${output}" />:</span>
              <form:input cssClass="search-query input-medium" placeholder="${cercaperoutput}" path="output" />
            </div>
