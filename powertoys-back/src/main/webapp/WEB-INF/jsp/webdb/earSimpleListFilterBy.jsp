...
<un:useConstants var="EarSimpleFields" className="org.fundaciobit.powertoys.model.fields.EarSimpleFields"/>
  ...
  <div id="FilterDiv" class="wellgroupfilter formbox" style="${displayFilterDiv} margin-bottom:3px; margin-left: 1px; padding:3px;">
      ...
      <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,EarSimpleFields.EARSIMPLEID)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="earSimple.earSimpleID" />:</span>

              <span class="add-on">&nbsp;<fmt:message key="genapp.from" /></span>
              
              <form:input cssClass="input-append input-small" path="earSimpleIDDesde" />


              <span class="add-on">&nbsp;<fmt:message key="genapp.to" />&nbsp;</span>

              <form:input cssClass="input-append input-small search-query" path="earSimpleIDFins" />

            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,EarSimpleFields.NOM)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="earSimple.nom" var="nom" />
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,EarSimpleFields.DETALL)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="earSimple.detall" var="detall" />
              <fmt:message key="genapp.form.searchby" var="cercaperdetall" >                
                 <fmt:param value="${detall}"/>
              </fmt:message>
              <span class="add-on"><c:out value="${detall}" />:</span>
              <form:input cssClass="search-query input-medium" placeholder="${cercaperdetall}" path="detall" />
            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,EarSimpleFields.DATA)}">
<%-- FILTRE DATE-TIME --%>
            <div class="input-group" style="padding-right:4px;padding-bottom:4px;align-items:center;">
              <span class="add-on"><fmt:message key="earSimple.data" />:</span>
              <span class="add-on">&nbsp;<fmt:message key="genapp.from" /></span>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,EarSimpleFields.JBOSSVERSION)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="earSimple.jbossVersion" var="jbossVersion" />
              <fmt:message key="genapp.form.searchby" var="cercaperjbossVersion" >                
                 <fmt:param value="${jbossVersion}"/>
              </fmt:message>
              <span class="add-on"><c:out value="${jbossVersion}" />:</span>
              <form:input cssClass="search-query input-medium" placeholder="${cercaperjbossVersion}" path="jbossVersion" />
            </div>
