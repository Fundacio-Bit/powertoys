...
<un:useConstants var="VersioFields" className="org.fundaciobit.powertoys.model.fields.VersioFields"/>
  ...
  <div id="FilterDiv" class="wellgroupfilter formbox" style="${displayFilterDiv} margin-bottom:3px; margin-left: 1px; padding:3px;">
      ...
      <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,VersioFields.VERSIOID)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="versio.versioID" />:</span>

              <span class="add-on">&nbsp;<fmt:message key="genapp.from" /></span>
              
              <form:input cssClass="input-append input-small" path="versioIDDesde" />


              <span class="add-on">&nbsp;<fmt:message key="genapp.to" />&nbsp;</span>

              <form:input cssClass="input-append input-small search-query" path="versioIDFins" />

            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,VersioFields.ENTORNAPLICACIOID)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="versio.entornAplicacioID" />:</span>

              <span class="add-on">&nbsp;<fmt:message key="genapp.from" /></span>
              
              <form:input cssClass="input-append input-small" path="entornAplicacioIDDesde" />


              <span class="add-on">&nbsp;<fmt:message key="genapp.to" />&nbsp;</span>

              <form:input cssClass="input-append input-small search-query" path="entornAplicacioIDFins" />

            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,VersioFields.VERSIO)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="versio.versio" var="versio" />
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,VersioFields.BUILD)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="versio.build" var="build" />
              <fmt:message key="genapp.form.searchby" var="cercaperbuild" >                
                 <fmt:param value="${build}"/>
              </fmt:message>
              <span class="add-on"><c:out value="${build}" />:</span>
              <form:input cssClass="search-query input-medium" placeholder="${cercaperbuild}" path="build" />
            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,VersioFields.DATA)}">
<%-- FILTRE DATE-TIME --%>
            <div class="input-group" style="padding-right:4px;padding-bottom:4px;align-items:center;">
              <span class="add-on"><fmt:message key="versio.data" />:</span>
              <span class="add-on">&nbsp;<fmt:message key="genapp.from" /></span>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,VersioFields.ALTRESDADES)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="versio.altresDades" var="altresDades" />
              <fmt:message key="genapp.form.searchby" var="cercaperaltresDades" >                
                 <fmt:param value="${altresDades}"/>
              </fmt:message>
              <span class="add-on"><c:out value="${altresDades}" />:</span>
              <form:input cssClass="search-query input-medium" placeholder="${cercaperaltresDades}" path="altresDades" />
            </div>
