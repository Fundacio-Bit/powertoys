...
<%@ include file="/WEB-INF/jsp/moduls/includes.jsp"%>
          ...
          <td>
          <c:set var="pk" value="${aplicacio.aplicacioID}"/>
          <c:choose>
           <c:when test="${__theFilterForm.actionsRenderer == 1}">
            <div class="btn-group" role="group" >
            <c:if test="${__theFilterForm.editButtonVisible}">
            <a class="btn btn-warning" href="<c:url value="${contexte}/${aplicacio.aplicacioID}/edit"/>" role="button"  title="<fmt:message key="genapp.edit"/>">
               <i class="fas fa-edit"></i>
            </a>
            </c:if>
            <c:if test="${__theFilterForm.viewButtonVisible}">
            <a class="btn btn-info" href="<c:url value="${contexte}/view/${aplicacio.aplicacioID}"/>" role="button"  title="<fmt:message key="genapp.viewtitle"/>">
               <i class="fas fa-eye"></i>
            </a>
            </c:if>
            <c:if test="${__theFilterForm.deleteButtonVisible}">
            <a class="btn btn-danger" href="#myModal" role="button"  onclick="openModal('<c:url value="${contexte}/${aplicacio.aplicacioID}/delete"/>','show');" title="<fmt:message key="genapp.delete"/>">
  ...
  <ul class="dropdown-menu float-right" style="min-width:35px;padding:5px 5px 0px 5px;margin:0px;font-size: 12px" >
            <c:if test="${__theFilterForm.editButtonVisible}">
            <li>
            <a class="btn btn-warning btn-sm a_item" style="margin-bottom:5px;color: white;" href="<c:url value="${contexte}/${aplicacio.aplicacioID}/edit"/>" onclick="null">
            <i class="fas fa-edit"></i>
            ...
            <c:if test="${__theFilterForm.viewButtonVisible}">
            <li>
            <a class="btn btn-info btn-sm a_item" style="margin-bottom:5px;color: white;" href="<c:url value="${contexte}/view/${aplicacio.aplicacioID}"/>" onclick="null">
            <i class="fas fa-eye"></i>
            ...
            <c:if test="${__theFilterForm.deleteButtonVisible}">
            <li>
            <a class="btn btn-danger btn-sm a_item" style="margin-bottom:5px;color: white;" href="#myModal" onclick="openModal('<c:url value="${contexte}/${aplicacio.aplicacioID}/delete"/>','show');">
            <i class="fas fa-trash icon-white"></i>
