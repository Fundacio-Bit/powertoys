...
<un:useConstants var="EarFields" className="org.fundaciobit.powertoys.model.fields.EarFields"/>
        ...
        <c:if test="${ __entry.key < 0  && ((empty __entry.value.searchBy)? true : !gen:contains(__theFilterForm.hiddenFields, __entry.value.searchBy)) && ((empty __entry.value.groupBy )? true : !gen:contains(__theFilterForm.hiddenFields, __entry.value.groupBy ))}">
          <td>
             <c:if test="${not empty __entry.value.valueMap }">
               <c:out escapeXml="${__entry.value.escapeXml}" value="${__entry.value.valueMap[ear.earID]}" />
             </c:if>
        ...
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,EarFields.FITXERID)}">
          <td>
            <c:if test="${not empty ear.fitxer}">
              <a target="_blank" href="<c:url value="${pwt:fileUrl(ear.fitxer)}"/>">${ear.fitxer.nom}</a>
            </c:if>
           </td>
        </c:if>
